 ...
 */
com.setCommonCode = function(_$p, codeJsonArr, callbackFuncStr, callbackFuncParamArr){
	var codeJsonArrLen;
	if(codeJsonArr){
		 codeJsonArrLen = codeJsonArr.length;
	}else{
		_$p.log("=== com.setCommonCode Parameter Type Error ===\nex) com.setCommonCode([{\"code:\":\"04\",\"compID\":\"sbx_Gender\"}],\"com.callbackFunction\")\n===================================");
		return;
	}
	...
	for(i=0;i<codeJsonArrLen;i++){
		...
		try{
			...
			try{
				_$p.data.remove(dltId);
			}catch(ex){}
			
			_$p.data.create(dcFormObj);
			...
			if(codeObj.compID){	//컴포넌트의 ID가 없는 경우도 있음.
				...
				for(j=0;j<compArrLen;j++){
					...
					var comObj = _$p.getComponentById(tmpIdArr[0]);
					try{
						...
						_$p.log(tmpIdArr[0] + "이 존재하지 않습니다.");
					}
				}
			}
			
		}catch(ex){
			_$p.log("com.setCommonCode Error");
			_$p.log(JSON.stringify(codeObj));
			_$p.log(ex);
			continue;
		}
	}
    ...
    var searchCodeGrpOption = {
    	id : "sbm_SearchCode",
    	action : "/data/sampleData/sampleData_json_dc_allCode.json",
    ...
    com.executeSubmission_dynamic(_$p, searchCodeGrpOption,{"dma_Common_Code":{"GRP_CD" : paramCode, "DATA_PREFIX" : dataPrefix}});
};
 ...
 * 		,submitDoneHandler : "searchCodeGrpCallback", isShowMeg : false };d
 * executeSubmission_dynamic($p, searchCodeGrpOption);
 */
com.executeSubmission_dynamic = function(_$p, options, requestData , obj) {
    ...
    if (submissionObj === null) {
    	...
    	com.createSubmission(_$p, options);
    } else {
    	com.setSubmission(submissionObj, options);
    }
    _$p.executeSubmission( options.id, requestData, obj);
 ...
 * @author InswaveSystems
 * @example
 * com.createSubmission($p, options);
 */
com.createSubmission = function(_$p, options) {
